...
const Scoretemp = {
  ...
  getOneScoretemp_to_YearAndObject: (ObjectId, YearId) => {
    ...
    WHERE s.IsDeleted = 0 AND s.ObjectId IN(?) AND s.yearId = ?`;
      connection.query(query, [ObjectId, YearId], (err, results) => {
        if (err) {
          reject(err);
        } else {
          resolve(results);
  ...
  getOne_InfoScoretemp: (Id) => {
    ...
    o.NameObject AS NameObject,
    c.Name AS NameCriteria,c._id AS Criteria_id,c.FieldId AS FieldId,
    d.Name AS NameCriteriaDetail,d._id AS CriteriaDetail_id,d.Score AS Score,d.Target AS Target,d.IsTypePercent AS IsTypePercent,d.IsTypeTotal AS IsTypeTotal,d.IsCurrentStatusType AS IsCurrentStatusType,d._id AS ScoretempDetail_id
    FROM scoretemp s
    JOIN year y ON s.YearId = y._id
    JOIN object o ON s.ObjectId = o._id
    LEFT JOIN criteria c ON s._id = c.ScoretempId
  ...
  getScoreTemp_YearAndObject_to_get_id: (ObjectId, YearId) => {
    return new Promise((resolve, reject) => {
  ...
  getObjectId_scoretempIdAndYearId: (ScoretempId, YearId) => {
    return new Promise((resolve, reject) => {
      const query = `SELECT * FROM scoretemp WHERE _id IN(?) AND YearId = ? AND IsDeleted = 0`;
      connection.query(query, [ScoretempId, YearId], (err, results) => {
        if (err) {
          reject(err);
        } else {
          resolve(results);
